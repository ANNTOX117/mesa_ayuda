...
class Principal extends MX_Controller {
    ...
    public function ticket_abierto(){
        if($this->session->userdata('logueado') && $this->session->userdata('rol_ua') != '1'){
            ...
            $where=$this->session->userdata('area_id_ua');
            $wherem=$this->session->userdata('id_mesabb');
            //var_dump($datos->get_ticket());
            $data = array();
            
            $data=[
                
                'areas'     =>$datos->get_areas($wherem),
            ...
            //var_dump($datos->get_areas($wherem));
            $data = [
                'navbar'        => $this->load->view('templates/vertical/navbar2_view', $data, TRUE),
 ...
 */
    ...
    public function insert_area(){
        ...
        redirect('areas/principal/ticket');
    ...
    public function insert_area2(){
        ...
        $url='areas/principal/responder?pregunta='.$this->input->post('ID_ticket');
        redirect($url);
        }
    }
    ...
    public function responder_insert(){
        ...
        //var_dump($respuesta);
        $url='areas/principal/responder?pregunta='.$respuesta['id_ticket'];
